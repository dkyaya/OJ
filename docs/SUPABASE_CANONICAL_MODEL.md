...
`set_trade_idea_archived` is the only browser-callable archive/restore path. It runs in the caller's RLS context, owner-locks the row, compares the exact expected revision, changes only `deleted_at`, advances the revision, and leaves hard delete unavailable. A private trigger refuses to archive any idea with confirmed active or closed trade history. Archived ideas remain canonical and exportable but are excluded from active counts, opportunity mappings, and entry selection. Archive and restore fail closed offline rather than entering the draft retry queue.
