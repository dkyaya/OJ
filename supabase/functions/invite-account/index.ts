...
Deno.serve(async (req) => {
  ...
  if (Number(req.headers.get('content-length') || 0) > 1024) return json(req, { error: 'request_too_large' }, 413);
  ...
  try {
    ...
    const recent = await admin.from('account_invites').select('id', { count: 'exact', head: true }).eq('invited_by', user.id).gte('created_at', new Date(Date.now() - 60 * 60 * 1000).toISOString());
    if ((recent.count || 0) >= 5) return json(req, { error: 'rate_limited' }, 429);

    const rawBody = await req.text();
    if (new TextEncoder().encode(rawBody).byteLength > 1024) return json(req, { error: 'request_too_large' }, 413);
    const body = JSON.parse(rawBody) as Record<string, unknown>;
    if (Object.keys(body).some((key) => key !== 'email')) return json(req, { error: 'invalid_invitation' }, 400);
    ...
    if (existing.data?.status === 'accepted' || (existing.data?.status === 'pending' && new Date(existing.data.expires_at).getTime() > Date.now())) {
