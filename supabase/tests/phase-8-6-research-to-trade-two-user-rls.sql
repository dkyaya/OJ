...
begin
  select trade_record.id,trade_record.entry_context into trade_id,original_context
  from public.trades trade_record
  where trade_record.trade_idea_id='62000000-0000-0000-0000-000000000002';
  ...
  update public.trade_ideas idea_record
  set data=jsonb_set(idea_record.data,'{Thesis}','"Later synthetic thesis."'),
      revision=idea_record.revision+1
  where idea_record.id='62000000-0000-0000-0000-000000000002';
  if (select trade_record.entry_context->>'thesis' from public.trades trade_record where trade_record.id=trade_id)<>'Original synthetic thesis.' then raise exception 'Idea edit rewrote immutable entry thesis'; end if;
  if (select trade_record.status from public.trades trade_record where trade_record.id=trade_id)<>'active' then raise exception 'weaker Thesis Health changed Trade state'; end if;
end $$;
...
begin
  ...
  if exists (select 1 from public.trade_ideas idea_record where idea_record.id=inaccessible_idea) then raise exception 'blocked User C Idea survived'; end if;
  begin
