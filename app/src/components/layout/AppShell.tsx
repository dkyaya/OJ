...
export function AppShell({ current, dark, onTheme, onBuildIdea, children }: PropsWithChildren<{ current: AppPath; dark: boolean; onTheme: () => void; onBuildIdea: () => void }>) {
  ...
  return <div className="app-shell">
    ...
    <main id="main-content"><header className="topbar"><div><span className="eyebrow">Options Journey</span><strong>{primaryNavigation.find((item) => item.path === current)?.label || (current === '/settings' ? 'Settings' : 'Overview')}</strong></div><div className="top-actions"><CloudAccount /><button className="icon-button" onClick={onTheme} aria-label={dark ? 'Use light mode' : 'Use dark mode'}>{dark ? <Sun /> : <Moon />}</button><button className="icon-button" onClick={() => navigate('/settings')} aria-label="Open settings"><Settings /></button></div></header>{children}</main>
