...
export default function App() {
  ...
  const refresh = useCallback(async () => { try { const next = await loadWorkspace(); setWorkspace(next); setError(''); if (next.preferences?.theme === 'dark') setDark(true); if (next.preferences?.theme === 'light') setDark(false); } catch (cause) { setError(cause instanceof Error ? cause.message : 'OJ could not load.'); } finally { setLoading(false); } }, []);
  ...
  if (loading) return <div className="loading-screen" role="status" aria-live="polite"><div className="juice"><img src={`${import.meta.env.BASE_URL}brand/oj-logo-mark-white.svg`} alt="" /><i /><b /><b /></div><span>Loading OJ</span></div>;
