...
export function IdeasPage({ workspace, onBuildIdea, onRecordTrade, onSaved, initialFilter = 'all' }: { workspace: Workspace; onBuildIdea: () => void; onRecordTrade?: (idea: TradeIdea) => void; onSaved: () => void | Promise<void>; initialFilter?: IdeaFilter }) {
  ...
  return <div className="page" data-tour-id="ideas-shell"><PageHeader title="Ideas" subtitle="Research and compare trade setups." action={<button className="primary" onClick={onBuildIdea}><Plus size={17} />Build Idea</button>} />
    ...
    {ideas.length ? <div className="idea-list" data-tour-id="idea-candidates">{ideas.map((idea) => {
      ...
      const catalyst = workspace.catalysts.find((item) => item.id === idea.catalystId || workspace.ideaCatalystLinks.some((link) => link.tradeIdeaId === idea.id && link.catalystId === item.id && link.relationship === 'primary'));
